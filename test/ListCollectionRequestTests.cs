...
{
  public class ListCollectionRequestTests
  {
    private readonly ITestOutputHelper output;

    private readonly string mockWebUrl = "https://mock.sharepoint.com/sites/mockSite";

    public ListCollectionRequestTests(ITestOutputHelper output)
    {
      this.output = output;
    }
    ...
    {
      // ARRANGE
      var responseContent = ResourceManager.GetHttpResponseContent("GetListsResponse.json");
      ...
      using (var gsc = GraphServiceTestClient.Create(responseMessage))
      {
        ...
        Assert.Equal("f12b876b-54d7-44e4-9dad-122bedd899a6", actual[0].Id);
        Assert.Equal("Documents", actual[0].Title);
        Assert.Equal(expectedChangeToken.StringValue, actual[0].CurrentChangeToken.StringValue);
        Assert.Equal(119, actual[2].BaseTemplate);
      }
    }
